...
import { OkxSpotRestClient } from '../rest/spot-rest.client';
import { OkxSpotWebsocketClient } from '../websocket/spot-websocket.client';
...
export class OkxSpotService implements IProduct {
  ...
  private transformOrder(data: any): Order {
    return {
      orderId: data.ordId?.toString(),
      symbol: data.instId.replace('-', '/'),
      side: data.side.toUpperCase(),
      type: data.ordType.toUpperCase(),
      quantity: parseFloat(data.sz),
      price: parseFloat(data.px),
      status: data.state,
      executedQty: parseFloat(data.accFillSz),
      cummulativeQuoteQty: parseFloat(data.accFillSz) * parseFloat(data.avgPx),
      avgPrice: parseFloat(data.avgPx),
